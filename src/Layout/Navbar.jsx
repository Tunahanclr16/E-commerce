...
export default function Navbar() {
  ...
  return (
    ...
    <CiUser size={22} className='cursor-pointer' onClick={() => setUserMenuOpen(!userMenuOpen)} />
    {userMenuOpen && (
      <div className='origin-top-right z-10 cursor-pointer absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white ring-2 ring-black ring-opacity-5'>
        <div className='py-1'>
          <p className='block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100'>Login</p>
          <p className='block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100'>Register</p>
        </div>
      </div>
    )}
    </div>        
